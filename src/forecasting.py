...
utilities to emulate the CarbonCast 96 hour outlooks that introduce realistic
noise into the carbon intensity signal.
"""
...
def _carbon_cast_noise(region: str, rng_seed: int) -> np.ndarray:
    """Cache noise vectors to avoid re-drawing them when iterating."""
    ...
    return np.concatenate(np.array(noise_daily))


def generate_carbon_cast_96hrs(C: np.ndarray, region: str, rng: np.random.Generator) -> np.ndarray:
    """Return a noisy 96h carbon intensity outlook inspired by CarbonCast."""

    noise_96hrs = _carbon_cast_noise(region, rng.integers(0, 2**32 - 1))
    return C * (1 + noise_96hrs[: len(C)])


def _compute_sd_gauss(mape: float) -> float:
    """Standard deviation of a zero-mean Gaussian that yields ``mape``."""
